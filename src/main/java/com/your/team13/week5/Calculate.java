...
public class Calculate {
	...
	public double totalRate() {
		...
		if(this.isFamliyDiscount()) {
			sum += customer.plan.getbasicMonthlyRate() + (customer.plan.getadditionalLineRate() * 2) + ((customer.getlineNumber() - 3) * 5);
		}
		else {
			sum += customer.plan.getbasicMonthlyRate() + (customer.plan.getadditionalLineRate() * (customer.getlineNumber() - 1));
		}
